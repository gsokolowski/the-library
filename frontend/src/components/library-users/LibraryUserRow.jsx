/**
 * @param {{
 *   user: { id: string | number; name: string; surname: string; email: string };
 *   onView: () => void;
 *   onEdit: () => void;
 *   onDelete: () => void;
 * }} props
 */
export default function LibraryUserRow({ user, onView, onEdit, onDelete }) {
    return (
        <li className="flex items-start justify-between gap-4 px-4 py-4 first:rounded-t-xl last:rounded-b-xl">
            <div>
                ...
                <p className="mt-1 text-sm text-stone-600 dark:text-stone-400">{user.email}</p>
                <p className="mt-2 font-mono text-xs text-stone-400 dark:text-stone-500">id {user.id}</p>
            </div>
